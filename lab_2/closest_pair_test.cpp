...
namespace Catch {
    ...
    struct StringMaker<pair<Point, Point>> {
    	static string convert(const std::pair<Point, Point> &p) {
    		ostringstream out;
...
TEST_CASE( "No points" ) {    
    CHECK_THROWS( closest_pair_fast({}) );    
}

TEST_CASE( "Single point" ) {    
    CHECK_THROWS( closest_pair_fast({Point(1, 1)}) );
}

TEST_CASE( "Two points" ) {    
    CHECK( ordered(closest_pair_fast({
    	Point(2, 3), Point(3, 4)})) == std::make_pair(Point(2, 3), Point(3, 4)) );
}

TEST_CASE( "Three points" ) {
    CHECK( ordered(closest_pair_fast({
    	Point(2, 3), Point(1, 9), Point(6, 2)})) == std::make_pair(Point(2, 3), Point(6, 2)) );
}

TEST_CASE( "Duplicate points" ) {    
    CHECK( ordered(closest_pair_fast({
    	Point(2, 3), Point(2, 3), Point(3, 4)})) == std::make_pair(Point(2, 3), Point(2, 3)) );
}

TEST_CASE( "Same x coordinate" ) {    
    CHECK( ordered(closest_pair_fast({
        Point(2, 9), Point(2, 4), 
        Point(2, 1), Point(2, -8)})) == std::make_pair(Point(2, 1), Point(2, 4)) );
}

TEST_CASE( "Many points" ) {
    CHECK( ordered(closest_pair_fast({
    	Point(2, 3), Point(0, 4), Point(11, 9), Point(2, 8),
    	Point(4, 4), Point(3, 6), Point(6, 5), Point(1, 9)})) == std::make_pair(Point(1, 9), Point(2, 8)) );
}

TEST_CASE( "Negative points" ) {
    CHECK( ordered(closest_pair_fast({
    	Point(-5, 6), Point(1, 2), Point(4, -2), Point(-9, 0),
    	Point(-1, -2), Point(0, 7), Point(2, -1), Point(-3, 1)})) == std::make_pair(Point(2, -1), Point(4, -2)) );
}
...
TEST_CASE( "Closest points are from stripe" ) {
    CHECK( ordered(closest_pair_fast({
        ...
        Point(10, 21), Point(10.5, 11), Point(11.7, -9), Point(12.2, -19)})) == std::make_pair(Point(-1, 20), Point(1, 21)) );
}
